...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_data', default='data/cat.npz',
...
class DataLoader():
    ...
    def make_batch(self, nbr_datum, use_cuda, type_set='train'):
...
class Model():
    ...
    def train(self, epoch, dataloader):
        ...
        if self.parametrization == 'point':
            # TODO: data is a global variable, there should be a DataLoader!!
            # batch_idx = np.random.choice(len(data),
            #                              self.hyper_params.batch_size)
            # batch_sequences = [data[idx] for idx in batch_idx]
            # batch, lengths = make_batch_point(batch_sequences,
            #                                  max_len_out, use_cuda)
            batch, lengths = dataloader.make_batch(self.hyper_params.batch_size,
                                                  use_cuda)
        elif self.parametrization == 'line':
            batch, lengths = make_batch_line(self.hyper_params.batch_size,
                                             data, max_len_out, use_cuda)
        # encode:
        z, self.mu, self.sigma = self.encoder(batch, self.hyper_params.batch_size)

        if self.parametrization == 'point':
            # create start of sequence:
            if use_cuda:
                sos = torch.stack([torch.Tensor([0, 0, 1, 0, 0])] *
                                  self.hyper_params.batch_size).cuda().unsqueeze(0)
            else:
                sos = torch.stack([torch.Tensor([0, 0, 1, 0, 0])] *
                                  self.hyper_params.batch_size).unsqueeze(0)
            # had sos at the begining of the batch:
            batch_init = torch.cat([sos, batch], 0)
            # expend z to be ready to concatenate with inputs
            z_stack = torch.stack([z]*(self.hyper_params.max_len_out+1))
            # TODO: Start Of Sequence not clear. Potential source of error.
        elif self.parametrization == 'line':
            if use_cuda:
    ...
    def conditional_generation_point(self, epoch,
        ...
        # data_to_batch = [data[idx]]
        # batch, lengths = make_batch_point(data_to_batch, max_len_out, use_cuda)
    ...
    def conditional_generation_line(self, epoch):
        # TODO: adapt that..
        batch, lengths = make_batch_line(1, data, max_len_out, use_cuda)
        print(batch.shape)
        # should remove dropouts:
        self.encoder.train(False)
        self.decoder.train(False)
        ...
        z, _, _ = self.encoder(batch, 1)
        if use_cuda:
            sos = torch.Tensor([0, 0, 0, 0, 0]).view(1, 1, -1).cuda()
        else:
            sos = torch.Tensor([0, 0, 0, 0, 0]).view(1, 1, -1)
        s = sos
        ...
        for i in range(max_len_out):
            ...
            # decode:
            self.pi, self.mu_x, self.mu_y, self.sigma_x, self.sigma_y, \
                self.rho_xy, self.pi_r, self.mu_r, self.sigma_r, self.pi_phi,\
                self.mu_phi, self.sigma_phi, self.q0,\
                hidden, cell = self.decoder(input, z, hidden_cell)
                # TODO: the batch_size should probably be a parameter?
            hidden_cell = (hidden, cell)
            # sample from parameters:
            # TODO: S represente next state
            s, dx, dy, r, phi, eos = self.sample_next_state_line()
            # ------
            seq_x.append(dx)
            seq_y.append(dy)
            seq_r.append(r)
        ...
        x_sample = np.cumsum(seq_x, 0)
        y_sample = np.cumsum(seq_y, 0)
        r_sample = np.array(seq_r)
        phi_sample = np.array(seq_phi)
        sequence = np.stack([x_sample, y_sample, r_sample, phi_sample]).T
        make_image_seq(sequence, epoch, dest_folder='images/train_line')
...
def sample_bivariate_normal(mu_x, mu_y, sigma_x, sigma_y,
    ...
    cov = [[sigma_x.item() * sigma_x.item(), rho_xy.item() * sigma_x.item() * sigma_y.item()],
           [rho_xy.item() * sigma_x.item() * sigma_y.item(), sigma_y.item() * sigma_y.item()]]
